...
async def admin_api_football_import_fixtures(data: AFImportFixturesIn, _staff=Depends(require_staff)):
    ...
    logger.info(f"IMPORT START: fixtures={len(fixtures)}")
    ...
    for item in fixtures:
        logger.info(f"IMPORT FIXTURE: {(item.get('fixture') or {}).get('id')}")
        fixture = item.get("fixture") or {}
        league = item.get("league") or {}
        teams = item.get("teams") or {}
        goals = item.get("goals") or {}
        ...
        # upsert teams
        before_count = teams_upserted
        await upsert_api_football_team(home)
        await upsert_api_football_team(away)
        # crude upsert count heuristic
        if home.get("id"):
            teams_upserted += 1
        if away.get("id"):
            teams_upserted += 1
        teams_upserted = max(teams_upserted, before_count)

        home_code = af_team_code(home.get("id"))
        away_code = af_team_code(away.get("id"))
        ...
        existing = await db.matches.find_one({"external_fixture_id": int(fixture_id)}, {"_id": 0})
        if existing:
            # update kickoff/metadata only (don't overwrite finished scores here)
            await db.matches.update_one({"id": existing["id"]}, {"$set": base_doc})
            updated += 1
        ...
        # apply result if fixture already finished
        try:
            if status in API_FOOTBALL_FINISHED_SHORT:
                h = goals.get("home")
                a = goals.get("away")
                if isinstance(h, int) and isinstance(a, int):
                    match_doc = await db.matches.find_one({"id": match_id}, {"_id": 0})
                    if match_doc and match_doc.get("status") != "finished":
...
async def on_shutdown():
    ...
    # API_FOOTBALL_IMPORT_INDEXES
    await db.matches.create_index("external_fixture_id", unique=True, sparse=True)
    await db.matches.create_index("competition")
    await db.matches.create_index("kickoff")
    await db.football_teams.create_index("code", unique=True)
    await db.football_teams.create_index("api_football_team_id", unique=True, sparse=True)
